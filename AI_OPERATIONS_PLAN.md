# AI Operations Plan — agy-sandbox (Starfall: Living Galaxy)
...
| Code host & gate | **GitHub** (`michaelcrosato/agy-sandbox`) with branch protection + **claude-code-action v1 (GA)** for `@claude` mentions and CI autofix | PRs are the unit of review; CI is the objective referee; `@claude` lets the operator turn a plain-English comment into a fix without opening anything else. |
| App hosting / QA surface | **Cloudflare Tunnel (cloudflared)** — instant public HTTPS URL for the Node/WebSocket server; staging runs `node src/server.js` and shares a tunnel URL for QA without a cloud deploy provider | A non-technical human QAs by clicking a URL, never by running anything. |
| Database | **Built-in JSON file store (Redis optional for multi-host scale)** — the server ships a swappable persistence layer; development uses a local JSON store; Redis is wired in when multi-shard scale work begins | Keeps destructive experimentation off production. Isolated test stores (in-memory or temp-dir JSON) keep E2E runs from touching the shared dev state file (§7.1). |
| E2E verification | **Playwright** driven by agents ("test like a human user") against the running server with seeded galaxy state — verifies the browser Canvas client end-to-end (fly, dock, trade, mission accept) | Anthropic's harness research found agents skip end-to-end verification unless given browser tooling and told to use it; this closed the biggest quality gap in their long-running trials. |
...
- **Allow:** `npm` scripts, `node`, `tsc`, `biome` (linter), `jest` / `vitest` / `playwright` (test runners), `git` (add/commit/push/branch/fetch/pull), `gh pr` equivalents via the GitHub integration, file edits repo-wide.
