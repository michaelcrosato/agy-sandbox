# AI Operations Plan — agy-sandbox
...
| E2E verification | **TBD** (e.g. Playwright, Cypress) driven by agents ("test like a human user") against seeded data | Anthropic's harness research found agents skip end-to-end verification unless given browser tooling and told to use it; this closed the biggest quality gap in their long-running trials. |
...
- **Allow:** `npm` scripts, `Node.js`, `tsc`, `biome`, `TBD` (test runner), `TBD` (DB CLI; dev instances only), `git` (add/commit/push/branch/fetch/pull), `gh pr` equivalents via the GitHub integration, file edits repo-wide.
